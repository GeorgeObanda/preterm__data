...
def signup(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Make account inactive until approved by admin
            user.is_active = False
            user.save()

            # Send email notification to all admins
            admins = User.objects.filter(role='AD', is_active=True)
            for admin in admins:
                ...
                send_mail(
                    subject=f"New Signup: {user.username} ({user.role})",
                    message=(
            ...
            messages.success(
                request,
                "Account created successfully. An admin will review and activate your account shortly."
            )
...
def choose_dashboard(request):
    if request.user.role != 'AD' and not request.user.is_superuser:
        return render(request, 'tracking/forbidden.html', {'message': "Only PIs or superusers can access this page."})

    participants = Participant.objects.all().order_by('-enrollment_date') if request.user.is_superuser else Participant.objects.filter(site=request.user.site).order_by('-enrollment_date')
    ...
    completed = [p for p in participants if not any([
        not p.monitor_downloaded, not p.ultrasound_downloaded, not p.case_report_form_uploaded,
        not p.video_laryngoscope_uploaded, not p.rop_final_report_uploaded,
        not p.head_ultrasound_images_uploaded, not p.head_ultrasound_report_uploaded,
        not p.cost_effectiveness_data_uploaded, not p.blood_culture_done,
        not p.admission_notes_day1_uploaded
    ])]

    return render(request, 'tracking/choose_dashboard.html', {'participants': participants, 'pending': pending_list, 'completed': completed})


# ---------- RA/RO Dashboard Views ----------
...
def ro_dashboard(request):
    ...
    sorted_participants = sorted(pending_participants, key=lambda p: p.days_remaining())
    return render(request, 'tracking/ro_dashboard.html', {'participants': sorted_participants})


# ---------- The rest of your views (PDF, Participant management, Quick mark, Blog) remain unchanged ----------
# You can keep all the existing code below as-is
# ---------- PDF Download ----------
...
def register_participant(request):
    if request.user.role not in ('RA', 'AD') and not request.user.is_superuser:
        return render(request, 'tracking/forbidden.html', {'message': "Only RAs, PIs, or superusers can register participants."})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not participant.site:
                participant.site = request.user.site if not request.user.is_superuser else None
            participant.save()
...
def update_participant(request, pk):
    ...
    # Permission check: only superusers, RO, or AD users from the same site can update
    if not request.user.is_superuser and (request.user.role not in ('RO', 'AD') or participant.site != request.user.site):
        return render(request, 'tracking/forbidden.html', {
            'message': "You can only modify participants from your site."
        })
    ...
    if request.method == "POST":
        ...
        checkbox_fields = [
            ...
            'admission_notes_day1_uploaded', 'rop_screening_done'
        ]
    ...
    # If accessed without POST, redirect to participant detail
